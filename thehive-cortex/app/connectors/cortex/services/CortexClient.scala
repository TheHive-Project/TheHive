...
class CortexClient(val name: String, baseUrl: String, authentication: Option[CortexAuthentication.Type], ws: CustomWSAPI) {
  ...
  def getAnalyzer(analyzerId: String)(implicit ec: ExecutionContext): Future[Analyzer] =
    request(s"api/analyzer/$analyzerId", _.get, _.json.as[Analyzer])
      .map(_.copy(cortexIds = List(name)))
  ...
  def getResponderByName(responderName: String)(implicit ec: ExecutionContext): Future[Responder] = {
    val searchRequest = Json.obj("query" → Json.obj("_field" → "name", "_value" → responderName), "range" → "0-1")
    request(s"api/responder/_search", _.post(searchRequest), _.json.as[Seq[Responder]])
      .flatMap { analyzers ⇒
        analyzers
          .headOption
  ...
  def getAnalyzerByName(analyzerName: String)(implicit ec: ExecutionContext): Future[Analyzer] = {
    val searchRequest = Json.obj("query" → Json.obj("_field" → "name", "_value" → analyzerName), "range" → "0-1")
    request(s"api/analyzer/_search", _.post(searchRequest), _.json.as[Seq[Analyzer]])
      .flatMap { analyzers ⇒
        analyzers
          .headOption
  ...
  def execute(responderId: String, label: String, dataType: String, data: JsValue, tlp: Long, pap: Long, message: String, parameters: JsObject)(
      implicit ec: ExecutionContext
  ): Future[JsValue] = {
    val body =
      Json.obj("label" → label, "data" → data, "dataType" → dataType, "tlp" → tlp, "pap" → pap, "message" → message, "parameters" → parameters)
  ...
  def status()(implicit ec: ExecutionContext): Future[JsObject] =
    for {
      ...
      Json.obj("name" → name, "version" → versionValue, "status" → status)
    }
