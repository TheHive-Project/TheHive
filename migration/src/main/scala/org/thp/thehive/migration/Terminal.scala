...
class Terminal(output: Writer) {
  lazy val pathedTput: String = if (new java.io.File("/usr/bin/tput").exists()) "/usr/bin/tput" else "tput"
...
object Terminal {
  ...
  lazy val pathedStty = if (new java.io.File("/bin/stty").exists()) "/bin/stty" else "stty"
