...
class CaseMergeSrv @Inject()(
  ...
  private[services] def baseFields(entity: BaseEntity): Fields =
    Fields(
      entity.attributes - "_id" - "_routing" - "_parent" - "_type" - "_seqNo" - "_primaryTerm" - "createdBy" - "createdAt" - "updatedBy" - "updatedAt" - "user"
    )
  ...
  private[services] def mergeArtifactsAndJobs(
      newCase: Case,
      cases: Seq[Case]
  )(implicit authContext: AuthContext, ec: ExecutionContext): Future[Done] = {
