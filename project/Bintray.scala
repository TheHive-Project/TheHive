...
object Bintray extends AutoPlugin {
  ...
  override lazy val projectSettings = Seq(
    ...
    publishLatest in ThisBuild := Def.taskDyn {
      ...
      if (latestName == file.getName)
        ...
        Def.task {
          removeVersion(bintrayEnsureCredentials.value, bintrayOrganization.value, "binary", bintrayPackage.value, latestVersion, sLog.value)
          btPublish(
            latestName,
            file,
            bintrayEnsureCredentials.value,
  ...
  private def asStatusAndBody =
    new FunctionHandler({ r =>
