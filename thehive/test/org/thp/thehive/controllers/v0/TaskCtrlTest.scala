  ...
  "task controller" should {
    "list available tasks and get one task" in testApp { app =>
      val taskId = app[Database].roTransaction { implicit graph =>
        app[TaskSrv].startTraversal.has("title", "case 1 task 1")._id.getOrFail("Task").get
      }
    ...
    "patch a task" in testApp { app =>
      ...
      val newTask = app[Database]
        .roTransaction { implicit graph =>
          app[TaskSrv].startTraversal.has("title", "new title task 1").richTask.getOrFail("Task")
        }
        .map(TestTask.apply)
    ...
    "unset task owner" in testApp { app =>
      ...
      val newTask = app[Database]
        .roTransaction { implicit graph =>
          app[TaskSrv].startTraversal.has("title", "case 1 task 1").richTask.getOrFail("Task")
        }
        .map(TestTask.apply)
