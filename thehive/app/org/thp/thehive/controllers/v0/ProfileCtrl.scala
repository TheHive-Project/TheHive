...
class ProfileCtrl @Inject()(entryPoint: EntryPoint, db: Database, profileSrv: ProfileSrv) extends QueryableCtrl {
  ...
  val outputQuery: Query = Query.output[Profile with Entity, OutputProfile]
    ...
    entryPoint("create profile")
      ...
      .authTransaction(db) { implicit request => implicit graph =>
        ...
        if (request.permissions.contains(Permissions.manageProfile))
          profileSrv.create(profile).map(createdProfile => Results.Created(createdProfile.toJson))
        else
  ...
  def update(profileId: String): Action[AnyContent] =
    entryPoint("update profile")
      .extract("profile", FieldsParser.update("profile", profileProperties))
      .authTransaction(db) { implicit request => implicit graph =>
