...
package object v0 {
  ...
  implicit def fromInputUser(inputUser: InputUser): User =
    inputUser
      ...
      .withFieldComputed(_.permissions, _.roles.flatMap(Permissions.withName)) // FIXME unkown permissions are ignored
      .transform
  ...
  implicit def toOutputUser(user: RichUser): Output[OutputUser] =
    Output[OutputUser](
      user
        .into[OutputUser]
        .withFieldComputed(_.roles, _.permissions.map(_.name).toSet)
  ...
  def outputCaseProperties(implicit db: Database): List[PublicProperty[Vertex, _, _]] =
    ...
    PublicPropertyListBuilder[CaseSteps, Vertex]
      ...
      .property[String]("resolutionStatus").derived(_ ⇒ _.outTo[CaseResolutionStatus], "name")
  ...
  def outputTaskProperties(implicit db: Database): List[PublicProperty[Vertex, _, _]] =
    ...
    .property[Option[String]]("owner").derived(_ ⇒ _.outTo[TaskUser], "login")
      .build
  // format: on
