...
class SearchCtrl(
  ...
  def search: Action[AnyContent] =
    entrypoint("search")
      ...
      .extract("from", FieldsParser[Long].optional.on("from"))
      .extract("to", FieldsParser[Long].optional.on("to"))
      .auth { implicit request =>
        val query: String = request.body("query")
        val from: Long    = request.body("from").getOrElse(0)
        val to: Long      = request.body("to").getOrElse(10)
        val results = db.source[JsValue] { implicit graph =>
          val filters = stdFields
            ...
            fullTextOnlyFields
              .toSeq
              .map(f => (_: Traversal[Vertex, Vertex, IdentityConverter[Vertex]]).unsafeHas(f, FullTextPredicate.contains(query)))
