...
class TaxonomyCtrl @Inject() (
  ...
  override val pageQuery: ParamQuery[OutputParam] =
    ...
    (range, traversal, authContext) =>
      traversal.richPage(range.from, range.to, range.extraData.contains("total"))(_.richTaxonomy(authContext))
  )
  override val outputQuery: Query =
    Query.outputWithContext[RichTaxonomy, Traversal.V[Taxonomy]]((traversal, authContext) =>
    traversal.richTaxonomy(authContext)
  )
  ...
  def importTaxonomy: Action[AnyContent] =
    ...
    .authPermittedTransaction(db, Permissions.manageTaxonomy) { implicit request => implicit graph =>
      ...
      // TODO Parse file & combine with body
      ...
      tags ++ predicateWithNoTags.map(p =>
        Tag(inputTaxo.namespace, p, None, None, tagSrv.defaultColour)
      )

      for {
        tagsEntities <- tags.toTry(t => tagSrv.create(t))
