...
//      actionBuilder: DefaultActionBuilder,
//      configuration: Configuration
//  ): Router = new TheHiveRouter(routerV0, routerV1, davRouter, assets, actionBuilder, configuration).get
//}
//
//@ProvidedBy(classOf[AssetProvider])
//class AssetGetter(get: String => Action[AnyContent]) {
//  def at(name: String): Action[AnyContent] = get(name)
//}
//
//class AssetProvider(environment: Environment, assets: Assets, extAssets: ExternalAssets) extends Provider[AssetGetter] {
//  lazy val logger: Logger = Logger(getClass)
//
